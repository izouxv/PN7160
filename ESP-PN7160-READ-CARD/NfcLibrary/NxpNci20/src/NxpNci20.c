...
void NFCEE_NDEF_Configuration(){
	uint8_t RF_SET_LISTEN_MODE_ROUTING_CMD[] = {0x21, 0x01, 0x22, 0x00, 0x05, 0x02, 0x09, 0x10, 0x3b, 0xd2, 0x76, 0x00, 0x00, 0x85, 0x01, 0x01, 0x03, 0x04, 0x00, 0x00, 0xfe, 0xfe, 0x01, 0x03, 0x00, 0x11, 0x04, 0x01, 0x03, 0x00, 0x01, 0x05,0x00, 0x03, 0x00, 0x39, 0x00};
	...
	NxpNci_HostTransceive(RF_SET_LISTEN_MODE_ROUTING_CMD, sizeof(RF_SET_LISTEN_MODE_ROUTING_CMD), Answer, sizeof(Answer), &AnswerSize);

	NxpNci_HostTransceive(NFCEE_DISCOVER_CMD, sizeof(NFCEE_DISCOVER_CMD), Answer, sizeof(Answer), &AnswerSize);
	NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize, TIMEOUT_INFINITE);

	NxpNci_HostTransceive(NFCEE_MODE_SET_CMD, sizeof(NFCEE_MODE_SET_CMD), Answer, sizeof(Answer), &AnswerSize);
	NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize, TIMEOUT_INFINITE);
	NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize, TIMEOUT_INFINITE);
}

void NFCEE_NDEF_DH_Write(){
	uint8_t Answer[MAX_NCI_FRAME_SIZE];
	uint16_t AnswerSize;

	//Commands => Check Standards for detailed explanation of each command
	uint8_t openConnection[] = {0x20, 0x04, 0x06, 0x03, 0x01, 0x01, 0x02, 0x10, 0x00};
	uint8_t applicationSelect[] = {0x05, 0x00, 0x0D, 0x00, 0xA4, 0x04, 0x00, 0x07, 0xD2, 0x76, 0x00, 0x00, 0x85, 0x01, 0x01, 0x00};
	uint8_t ccSelect[] = {0x05, 0x00, 0x07, 0x00, 0xA4, 0x00, 0x0C, 0x02, 0xE1, 0x03};
	//Read CC length
	uint8_t firstRead[] = {0x05, 0x00, 0x05, 0x00, 0xB0, 0x00, 0x00, 0x02};
	//Read content of CC file
	uint8_t secondRead[] = {0x05, 0x00, 0x05, 0x00, 0xB0, 0x00, 0x00, 0x0F};

	uint8_t NDEFSelect[] = {0x05, 0x00, 0x07, 0x00, 0xA4, 0x00, 0x0C, 0x02, 0xE1,0x04};
	//Write file size to 00
	uint8_t firstWrite[] = {0x05, 0x00, 0x07, 0x00, 0xD6, 0x00, 0x00, 0x02, 0x00,0x00};
	//Write NDEF message
	...
	//Write NDEF size
	uint8_t thirdWrite[] = {0x05, 0x00, 0x07, 0x00, 0xD6, 0x00, 0x00, 0x02, 0x00,0x12};

	uint8_t closeConnection[] = {0x20, 0x05, 0x01, 0x05};

    NxpNci_HostTransceive(openConnection, sizeof(openConnection), Answer, sizeof(Answer), &AnswerSize);
    NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize, TIMEOUT_1S);

    NxpNci_HostTransceive(applicationSelect, sizeof(applicationSelect), Answer, sizeof(Answer), &AnswerSize);
    NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize, TIMEOUT_1S);

    NxpNci_HostTransceive(ccSelect, sizeof(ccSelect), Answer, sizeof(Answer), &AnswerSize);
    NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize, TIMEOUT_1S);

    NxpNci_HostTransceive(firstRead, sizeof(firstRead), Answer, sizeof(Answer), &AnswerSize);
    NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize, TIMEOUT_1S);

    NxpNci_HostTransceive(secondRead, sizeof(secondRead), Answer, sizeof(Answer), &AnswerSize);
    NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize, TIMEOUT_1S);

    NxpNci_HostTransceive(NDEFSelect, sizeof(NDEFSelect), Answer, sizeof(Answer), &AnswerSize);
    NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize, TIMEOUT_1S);

    NxpNci_HostTransceive(firstWrite, sizeof(firstWrite), Answer, sizeof(Answer), &AnswerSize);
    NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize, TIMEOUT_1S);

    NxpNci_HostTransceive(secondWrite, sizeof(secondWrite), Answer, sizeof(Answer), &AnswerSize);
    NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize, TIMEOUT_1S);

    NxpNci_HostTransceive(thirdWrite, sizeof(thirdWrite), Answer, sizeof(Answer), &AnswerSize);
    NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize, TIMEOUT_1S);

    NxpNci_HostTransceive(closeConnection, sizeof(closeConnection), Answer, sizeof(Answer), &AnswerSize);
    NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize, TIMEOUT_1S);
}
